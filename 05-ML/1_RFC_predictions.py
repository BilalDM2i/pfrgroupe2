...
def create_organigram(winner, finalists, semi_finalists, quarter_finalists):
    ...
    fig.show()
    fig.write_image('RFC Tableau final.png')
...
plt.savefig('RFC_graphique_scores_tests.png')
...
X_predict = predict_df[['home_team_encoded', 'away_team_encoded', 'Home_Age_Moyen', 'Home_Taille_Moyenne',
                        ...
                        'Away_Taille_Moyenne', 'Away_Selection_Moyenne', 'Away_Buts_Moyens',
                        'Home_Rank', 'Away_Rank']]
predict_df['match_result_pred'] = best_rfc.predict(X_predict)
...
poules_predict_df.to_csv('RFC_predictions_poules.csv', index=False)
print("Les résultats prédits pour les matchs de poules ont été sauvegardéss dans 'RFC_predictions_poules.csv'.")
...
quarts_de_finale_data['match_result_pred'] = best_rfc.predict(X_quarts)
...
quarts_predict_df.to_csv('RFC_predictions_quarts.csv', index=False)
print("Les résultats prédits des quarts de finale ont été sauvegardés dans 'RFC_predictions_quarts.csv'.")
...
# Utiliser le modèle déjà entraîné pour prédire les résultats
demis_finale_data['match_result_pred'] = best_rfc.predict(X_demis)
...
demis_predict_df.to_csv('RFC_predictions_demis.csv', index=False)
print("Les résultats prédits des demi-finales ont été sauvegardés dans 'RFC_predictions_demis.csv'.")
...
# Utiliser le modèle déjà entraîné pour prédire les résultats
finale_data['match_result_pred'] = best_rfc.predict(X_finale)
...
finale_predict_df.to_csv('RFC_predictions_finale.csv', index=False)
print("Les résultats prédits de la finale ont été sauvegardés dans 'RFC_predictions_finale.csv'.")
