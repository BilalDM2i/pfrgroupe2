...
predict_df = pd.read_csv('quarts_de_finale.csv')
...
final_predict_df = predict_df[['Home_Team', 'Away_Team', 'home_result_pred', 'away_result_pred']]
print(final_predict_df)

# Sauvegarder les résultats prédits dans un nouveau fichier CSV
final_predict_df.to_csv('predicted_results_Q.csv', index=False)
print("Les résultats prédits ont été sauvegardés dans 'predicted_results_Q.csv'.")

#%%

# Initialiser une liste pour stocker les équipes gagnantes
gagnants_quarts = []

# Parcourir chaque match des quarts de finale
for idx, row in final_predict_df.iterrows():
    if row['home_result_pred'] == 1:
        gagnants_quarts.append(row['Home_Team'])
    elif row['away_result_pred'] == 1:
        gagnants_quarts.append(row['Away_Team'])

# Assurer qu'il y a exactement 4 gagnants
assert len(gagnants_quarts) == 4, "Il devrait y avoir exactement 4 équipes gagnantes."

# Créer le DataFrame pour les demi-finales
demi_finales_df = pd.DataFrame({
    'Date': [''] * 2,  # Ajouter une colonne Date vide si nécessaire
    'Home_Team': [gagnants_quarts[0], gagnants_quarts[2]],
    'Away_Team': [gagnants_quarts[1], gagnants_quarts[3]],
    'Resultat_Home_Team': [0, 0],  # Initialiser avec 0, car les demi-finales n'ont pas encore eu lieu
    'Resultat_Away_Team': [0, 0]   # Initialiser avec 0, car les demi-finales n'ont pas encore eu lieu
})

# Écrire les demi-finales dans un fichier CSV
demi_finales_df.to_csv('demi_finales.csv', index=False)

print("Les demi-finales ont été enregistrées dans le fichier 'demi_finales.csv'.")
