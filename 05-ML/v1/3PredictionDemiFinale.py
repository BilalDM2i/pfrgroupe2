...
predict_df = pd.read_csv('demi_finales.csv')
...
final_predict_df.to_csv('predicted_results_D.csv', index=False)
print("Les résultats prédits ont été sauvegardés dans 'predicted_results_D.csv'.")
...
for idx, row in final_predict_df.iterrows():
    if row['home_result_pred'] == 1:
        gagnants_demis.append(row['Home_Team'])
    elif row['away_result_pred'] == 1:
        gagnants_demis.append(row['Away_Team'])

# Créer le DataFrame pour la finale
finale_df = pd.DataFrame({
    'Date': [''],  # Ajouter une colonne Date vide si nécessaire
    'Home_Team': [gagnants_demis[0]],
    'Away_Team': [gagnants_demis[1]],
    'Resultat_Home_Team': [0],  # Initialiser avec 0, car les demi-finales n'ont pas encore eu lieu
    'Resultat_Away_Team': [0]   # Initialiser avec 0, car les demi-finales n'ont pas encore eu lieu
})

# Écrire les demi-finales dans un fichier CSV
finale_df.to_csv('finale.csv', index=False)

print("Les demi-finales ont été enregistrées dans le fichier 'finale.csv'.")
