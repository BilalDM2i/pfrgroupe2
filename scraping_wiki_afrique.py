...
url = "https://fr.wikipedia.org/wiki/Championnat_d%27Afrique_des_nations_masculin_de_handball"
...
table = soup.find(id="Palmar.C3.A8s").find_next('table')
...
df_palmares = pd.DataFrame(rows)
df_palmares.drop([3, 6, 7, 8, 9], axis=1, inplace=True)
df_palmares.columns = ['Année', 'Nation hôte', 'Champion', 'Finaliste', 'Troisième']
print(df_palmares.to_string())

# Retrait lignes totalement vides
df_palmares = df_palmares.dropna(how='all')
...
file_path_palmares = "resultats_scraping/palmares_handball_afrique_hommes.csv"
...
# -------------------------------------------------
table = soup.find(id="Tableau_d.27honneur").find_next('table')
...
df_tableau_medailles['Nation'] = df_tableau_medailles['Nation'].str.replace(r"/", "",
                                                                            regex=True)  # Supprimer les barres obliques
df_tableau_medailles['Nation'] = df_tableau_medailles[
    'Nation'].str.strip()  # Enlever les espaces blancs au début et à la fin

df_tableau_medailles.loc[df_tableau_medailles.Nation == "Total", "Rang"] = np.nan

# Afficher le DataFrame
print(df_tableau_medailles)

# Sauvegarder les données dans un fichier CSV
file_path_tab_medailles = "resultats_scraping/tableau_medailles_afrique_hommes.csv"
    ...
    while col_index < len(cells):
        ...
        if span:
            if span['data-sort-value'] == '0':
                cell_text = '1'
            elif span['data-sort-value'] == '77':
                cell_text = 'Disqualifié'
            elif span['data-sort-value'] == '88':
                cell_text = 'Forfait'
            elif span['data-sort-value'] == '99':
                cell_text = np.nan
            else:
...
# Sauvegarder les données dans un fichier CSV
file_path_bilan_nations = "resultats_scraping/bilan_nations_afrique_hommes.csv"
