...
train_df['match_result'] = train_df.apply(lambda x: 1 if x['Resultat_Home_Team'] > x['Resultat_Away_Team'] else (2 if x['Resultat_Home_Team'] == x['Resultat_Away_Team'] else 0), axis=1)
...
# Selection de colonnes // j'ai supprimé le winning team
poules_predict_df = predict_df[['Date', 'Home_Team', 'Away_Team', 'match_result_pred']]
...
for idx, row in poules_predict_df.iterrows():
    ...
    if match_result_pred == 1:
       points[Home_Team] += 2
    elif match_result_pred == 0:
       points[Away_Team] += 2
    else:
       points[Home_Team] += 1    
       points[Away_Team] += 1
