# Prédictions des poules puis quarts puis demi puis finales

# WARNING : revoir les commentaires et les noms de variables peut-être
...
def ajout_variables_train(matchs, train_df):
    data = []
    # Pour chaque match extraire les données correspondantes de train_df
...
# Créer une colonne qui indique si l'équipe à domicile a gagné (1) ou perdu (0)
train_df['match_result'] = (train_df['Resultat_Home_Team'] > train_df['Resultat_Away_Team']).astype(int)
# WARNING : on pourrait sinon ici mettre les résultats des matchs, les scores ?
...
# QUARTS
# ----------------------------------------------------------------------------------------------------------------------
# On fait maintenant les prédictions sur quarts_de_finale_df, il faut ajouter les infos pour faire la prédiction
...
# Selectionner uniquement les colonnes nécessaires pour les prédictions
quarts_predict_df = quarts_de_finale_data[['Home_Team', 'Away_Team', 'match_result_pred', 'Winning_Team']]

print("\nPrédiction des quarts de finale : \n", quarts_predict_df)
...
print("\nPrédiction des demis de finale : \n", demis_predict_df)

# Sauvegarder les résultats prédits dans un fichier CSV
demis_predict_df.to_csv('predictions_demis.csv', index=False)
print("Les résultats prédits des demis de finale ont été sauvegardés dans 'predictions_demis.csv'.")
