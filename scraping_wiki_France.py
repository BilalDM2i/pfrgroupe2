...
try:
    # Lire le tableau HTML en excluant la première ligne vide comme en-tête
...
columns_of_interest = ["No", "P.", "Nom", "Date de naissance", "Taille", "Poids", "Sél.", "Buts", "Club", "Depuis"]
...
print(df)
indices_to_remove = [0, 4, 10, 18]  # Par exemple, supprimer les lignes aux indices 3, 7, 14 et 17
df = df.drop(indices_to_remove)

# Sauvegarder les données dans un fichier CSV avec encodage UTF-8
os.makedirs("resultats_scraping", exist_ok=True)
file_path = "resultats_scraping/joueurs_equipe_france_handball.csv"
